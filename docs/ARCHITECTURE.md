...
### Bring your own data: user-uploaded documents

Retrieval isn't limited to the curated knowledge base — a user can upload their own CSV, TXT, MD, or PDF (`app/api/routes/documents.py`, `app/db/models/user_document.py`) and ABBot answers from it directly, everywhere it answers (chat, the floating widget, Practice Lab follow-ups), not just in one conversation:

- **Text extraction is format-specific** (`app/services/document_processing.py`): TXT/MD are read as-is; PDF text is extracted page by page via `pypdf`; **CSV is summarized into prose** (row/column counts, per-column stats or top categorical values, a sample of rows) rather than embedded row-by-row — raw numbers and category codes don't carry much meaning for cosine-similarity search, but a description of the data's shape does.
- **Chunked and embedded exactly like the curated KB** — `app/rag/chunking.py` holds the one `chunk_text()` implementation both paths share, so the two ingestion routes can't quietly drift into different chunking behavior.
- **One merged, re-ranked retrieval pool, not two separate ones** — `similarity_search()` in `app/rag/vector_store.py` queries the curated `kb_chunks` and, when a `user_id` is given, that user's own `user_document_chunks`, then merges both result sets and keeps the overall top-k by similarity (never top-k from each, which would silently double the context handed to the model). A citation's slug is prefixed `user-doc:` when it came from an upload, which is how the frontend (`GroundedIn` in `components/ui.tsx`) knows to route a "view source" click to `/api/documents/{id}` instead of `/api/kb/{slug}`, and to label it "Yours" rather than presenting it as part of the curated KB.
- **Scoped and bounded per account** — every query joins on `UserDocument.user_id == current_user.id`; one person's upload never surfaces in another account's answers. Capped at 20 documents and 5MB per file — generous for text/CSV, bounded to keep worst-case latency and Supabase storage predictable.
...
- **A metrics catalog** (`app/db/models/metric.py`, `/api/metrics`) — the mechanic Statsig's whole product is actually organized around: define what "Checkout Conversion" means once (a name, a description, the column it maps to, whether it's a guardrail), then reuse it by name on every future experiment instead of re-picking raw columns each time. The Advanced creation flow (`experiments/new/page.tsx`) cross-references an uploaded dataset's columns against the account's saved metrics client-side — a matching non-guardrail metric becomes the default primary metric selection, and matching guardrail metrics merge into the same suggestion set as the heuristic column-name detection. The experiment result page resolves the same way, so a Scorecard row reads "Checkout Conversion" instead of the raw column name `converted` wherever a saved metric applies.
- **A decision workflow** — `Experiment.decision` (`"shipped" | "rolled_back" | null`, set via `PATCH /api/experiments/{id}/decision`) makes an experiment a system of record with a real recorded outcome, not a number read once and forgotten. Shown as an action on the result page and a column in the Experiments list.
...
Every user shares one Groq API key, which means the free tier's 8000-tokens/minute ceiling is an app-wide budget, not a per-user one — worth taking seriously for something meant to run continuously, not just survive a demo. `chat_completion()` (`app/rag/llm_client.py`) was tuned against measured reality, not guesses:

- **`max_tokens=1800`**, set after measuring a real, fully-loaded exchange (system prompt + retrieved KB context + experiment context): a thorough answer runs ~1200 completion tokens, so 1800 covers that comfortably without inviting responses long enough to meaningfully eat into the shared per-minute budget. (This number went through two rounds of tuning — 700 truncated regularly, 2500 proved fast enough to exhaust the whole budget in two back-to-back long exchanges during live testing.)
- **`MAX_HISTORY_MESSAGES` trimmed from 12 to 8** (`app/api/routes/chat.py`) — with chat history now persisted and resent on every message, each extra message is real recurring input-token cost against the same shared budget.
- **A real retry on `RateLimitError`**, not just a caught exception: Groq's 429 response includes an `x-ratelimit-reset-tokens` header telling you exactly how long until the bucket refills. `chat_completion()` reads it, waits that long (capped at 10s) via `asyncio.sleep` — not `time.sleep`, since this function runs inside an async FastAPI route and a blocking sleep would stall every other request the same process is handling — and retries once. If it's still rate-limited, it returns a clear "ABBot is getting a lot of questions right now" message instead of a raw exception dump. `backend/tests/test_llm_client.py` exercises both the successful-retry and the still-rate-limited paths against a mocked Groq client, so this doesn't depend on actually exhausting the real quota to verify.
