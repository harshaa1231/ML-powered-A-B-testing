"""Statistical testing engine for A/B experiments.

Ported from the original `statistical_tests.py` prototype. Same math
(two-proportion z-test, Welch's t-test, Mann-Whitney U, chi-square),
now typed and unit-tested.
"""
...
class StatisticalTester:
    """Core statistical primitives plus adapter methods returning a uniform result dict."""
    ...
    def mann_whitney_u(control_values: np.ndarray, treatment_values: np.ndarray) -> TestResult:
        ...
        return TestResult(test_name="Mann-Whitney U", p_value=float(p_value), statistic=float(u_stat), extra={})
    ...
    def independent_ttest(self, control, treatment) -> dict[str, Any]:
        ...
        return {
            ...
            "n_control": int(np.asarray(control).shape[0]),
            "n_treatment": int(np.asarray(treatment).shape[0]),
        }
    ...
    def chi_square_test(self, control, treatment) -> dict[str, Any]:
        ...
        res = self.chi_square_2x2(c_succ, c_tot - c_succ, t_succ, t_tot - t_succ)
    ...
    def two_proportion_test(self, control_success: int, control_total: int, treatment_success: int, treatment_total: int) -> dict[str, Any]:
        res = self.two_proportion_z_test(control_success, control_total, treatment_success, treatment_total)
        p_control = res.extra["p_control"]
        p_treatment = res.extra["p_treatment"]
        uplift_pct = ((p_treatment - p_control) / p_control * 100.0) if p_control else 0.0
