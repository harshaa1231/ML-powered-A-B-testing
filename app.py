...
with st.sidebar:
    ...
    pages = ["Home", "Learn A/B Testing", "Simple A/B Test", "AB Test Analysis", "ML Model Studio", "Predictions", "Sample Datasets", "AI Chat"]
...
- How many items did they add to cart? (2.3 items)
    ...
    if "yes/no" in simple_mode:
        ...
        if conversions_a > visitors_a:
            ...
            st.session_state.predictions.append(simple_results_yesno)
        ...
        if st.button("Tell me the result", type="primary", width='stretch', key="simple_numeric_run"):
            ...
            st.session_state.predictions.append(simple_results_numeric)
...
**Feature Importance** — After training, the model tells you which columns (features) had the biggest influence on the prediction. This helps you understand *what drives results* in your data.
    ...
    st.markdown("---")
    st.subheader("Explore a Sample Dataset")
    explore_sample = st.selectbox("Pick a dataset to learn about:",
                                  ["Choose one..."] + [sample['name'] for sample in samples.values()],
                                  key="explore_sample_select")

    if explore_sample != "Choose one...":
        selected_sample = None
        for s in samples.values():
            if s['name'] == explore_sample:
                selected_sample = s
                break
        if selected_sample:
            with st.expander("Data Preview", expanded=True):
                st.dataframe(selected_sample['df'].head(15), width='stretch')
            sample_domain = "general"
